...
enum CalendarWeekStart: String, CaseIterable, Hashable, Identifiable {
    case sunday
    case monday
    ...
    init(firstWeekday: Int) {
        self = firstWeekday == CalendarWeekStart.monday.firstWeekday
            ? .monday
            : .sunday
    }
}
...
enum LocalDataOperationMessageKind: String, Equatable {
    case success
    case failure
}
...
struct CalendarEventEditorSession: Equatable, Identifiable {
    ...
    let editorDraft: CalendarEventDraft
    ...
    init(
        ...
        editorDraft: CalendarEventDraft? = nil,
        ...
        self.editorDraft = editorDraft ?? initialDraft
...
enum CalendarEventDropTarget: Equatable {
    case day(Date)
    case time(Date)
}
...
private struct PendingTaskTimeBlock: Equatable {
    ...
    let returnSection: WorkspaceSection
}
...
private enum EventMutationSafetyError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .linkedFutureSeriesDeferred:
            "This Event Brief is linked to one occurrence. BriefCal will not change or delete this and future occurrences until every affected Brief can be reconciled safely."
        case .confirmationUnavailable:
...
private enum LocalDataMaintenanceError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .unavailable:
            "BriefCal local data is unavailable, so this operation did not run."
        case .operationInProgress:
            "Another local data operation is already in progress."
        case let .recoveryRequired(message):
            message
        case .unsavedNotes:
            "BriefCal could not save every open notes draft. Retry the failed draft before backing up, importing, or resetting local data."
        case .interactionInProgress:
            "Finish or close the event editor, recovery review, relink check, or Undo operation before replacing local data."
        case .automaticBackupLocationUnavailable:
            "BriefCal could not determine a safe location for the automatic recovery backup. Nothing was changed."
        }
    }
}
...
final class AppState: ObservableObject {
    private static let calendarWeekStartPreferenceKey =
        "BriefCal.calendarWeekStart"

    @Published var selectedSection: WorkspaceSection? = .week
    @Published var rightSidebarMode: RightSidebarMode = .eventDetails
    ...
    @Published private(set) var calendarWeekStart: CalendarWeekStart
    ...
    @Published private(set) var localDataOperationMessageKind:
        LocalDataOperationMessageKind?
    ...
    init(
        ...
        let storedWeekStart = preferences.string(
            forKey: Self.calendarWeekStartPreferenceKey
        ).flatMap(CalendarWeekStart.init(rawValue:))
        let calendarWeekStart = storedWeekStart
            ?? CalendarWeekStart(firstWeekday: calendar.firstWeekday)
        var configuredCalendar = calendar
        configuredCalendar.firstWeekday = calendarWeekStart.firstWeekday
        ...
        focusedDate = configuredCalendar.startOfDay(for: now())
    ...
    private var localDataMaintenanceBlockError: LocalDataMaintenanceError {
        ...
        return .recoveryRequired(
            localDataQuarantineMessage
                ?? "BriefCal local data is locked for this session after a failed rollback. Quit BriefCal before attempting recovery."
        )
    }
    ...
    var visibleEvents: [DisplayEvent] {
        displayedEvents(in: visibleInterval)
    }

    func displayedEvents(in interval: DateInterval) -> [DisplayEvent] {
    ...
    var focusedPeriodTitle: String {
        ...
        case .agenda:
            return WorkspaceSection.agenda.title
        case .month:
    ...
    func setCalendarWeekStart(_ weekStart: CalendarWeekStart) {
        preferences.set(
            weekStart.rawValue,
            forKey: Self.calendarWeekStartPreferenceKey
        )
        guard calendarWeekStart != weekStart
                || calendar.firstWeekday != weekStart.firstWeekday else {
            return
        }

        var updatedCalendar = calendar
        updatedCalendar.firstWeekday = weekStart.firstWeekday
    ...
    func openTaskDeepLink(_ url: URL) async {
        guard url.scheme?.lowercased() == "briefcal",
        ...
        do {
            ...
            case .candidates, .ambiguous, .inconclusive:
                ...
                select(.tasks)
                localOperationError = "BriefCal could not safely identify the calendar event for this task link. Choose a replacement event to relink it."
            }
        } catch {
    ...
    /// Calendars represented by the active sidebar filter. Saved Sets keep
    /// their membership order, while Smart Role filters keep provider order.
    /// Disabled calendars remain listed so users can see and re-enable them.
    var selectedCalendarSetSources: [CalendarSource] {
        switch selectedCalendarSet {
        case .all:
        ...
        case let .saved(setID):
            guard let set = savedCalendarSet(id: setID) else { return [] }
            let sourcesByID = Dictionary(
                uniqueKeysWithValues: calendarSources.map { ($0.id, $0) }
            )
            return set.memberships.compactMap {
                sourcesByID[$0.calendarIdentifier]
            }
        }
    }
        ...
        case .all:
            if !calendarSources.isEmpty,
               calendarSources.allSatisfy({
                ...
                return "All calendars are disabled in BriefCal"
            }
        case let .role(role):
            ...
            if matchingCalendars.allSatisfy({
                ...
                return "All \(role.title) calendars are disabled in BriefCal"
            }
        case let .saved(setID):
            ...
            if availableMembers.allSatisfy({
                ...
                return "All calendars in \(set.name) are disabled in BriefCal"
            }
        }
        return "No events in this period"
    }
    ...
    func clearLocalDataOperationMessage() {
        ...
        localDataOperationMessageKind = nil
    }
    ...
    func exportLocalDataBackup(
        ...
        do {
            ...
            setLocalDataOperationMessage(
                "Backup did not run. \(Self.message(for: error))",
                kind: .failure
            )
            return nil
        }
        ...
        do {
            ...
            setLocalDataOperationMessage(
                "Backup exported to \(result.archiveURL.path(percentEncoded: false)). The ZIP is not encrypted.",
                kind: .success
            )
            return result
        } catch {
            endLocalDataOperation(reloadLocalProjections: false)
            setLocalDataOperationMessage(
                "Backup failed. No new partial backup was committed; an existing destination, if any, was left unchanged. \(Self.message(for: error))",
                kind: .failure
            )
            return nil
        }
    ...
    func importLocalDataBackup(
        ...
        do {
            ...
            setLocalDataOperationMessage(
                "Import did not run. \(Self.message(for: error))",
                kind: .failure
            )
            return nil
        }
        ...
        do {
            ...
            setLocalDataOperationMessage(
                "Backup imported. The previous local database was saved to \(result.automaticBackupURL.path(percentEncoded: false)). Calendar and Exchange events were not changed.",
                kind: .success
            )
            return result
        } catch {
            ...
            endLocalDataOperation(reloadLocalProjections: false)
            setLocalDataOperationMessage(
                "Import failed validation or restore. The active local database was kept or rolled back. \(Self.message(for: error))",
                kind: .failure
            )
            return nil
        }
    ...
    func resetLocalData() async -> LocalDataResetResult? {
        ...
        do {
            ...
            setLocalDataOperationMessage(
                "Reset did not run. \(Self.message(for: error))",
                kind: .failure
            )
            return nil
        }
        ...
        do {
            ...
            endLocalDataOperation(reloadLocalProjections: true)
            setLocalDataOperationMessage(
                "Active BriefCal local data was reset. A recovery backup remains at \(result.automaticBackupURL.path(percentEncoded: false)). Calendar and Exchange events were not changed.",
                kind: .success
            )
            return result
        } catch {
            ...
            endLocalDataOperation(reloadLocalProjections: false)
            setLocalDataOperationMessage(
                "Reset failed. The active local database was kept or rolled back. \(Self.message(for: error))",
                kind: .failure
            )
            return nil
        }
    ...
    func deleteTaskCenterItemAfterRemoteDelete(
        ...
        guard let contextStore else {
            localOperationError = "BriefCal local data is unavailable."
            return false
        }
        do {
            taskProviderCoordinator?.requestProviderSync()
        ...
        do {
            ...
            case let .inconclusive(issue):
                ...
                case .calendarUnavailable:
                    localOperationError = "The saved calendar is currently unavailable, so BriefCal did not count this as a missing event. The local Event Brief was kept."
                ...
                case .recurringOccurrenceUnavailable:
                    localOperationError = "The recurring series is visible, but the exact occurrence could not be proven inside the safe search bounds. BriefCal did not mark it missing. Choose the occurrence manually if it moved."
                case .strongIdentifierOccurrenceMismatch:
                    localOperationError = "A strongly identified calendar item still exists, but its recurrence or occurrence no longer matches the saved link. BriefCal did not mark it missing. Choose the exact occurrence manually."
                }
                linkedEventRecoverySession = LinkedEventRecoverySession(
                    brief: target.brief,
                    stage: .manualRelink
    ...
    func beginCreatingTaskTimeBlock(
        ...
        guard sidebarTaskReference.hasPrefix("briefcal-task:"),
              let coordinator = taskProviderCoordinator,
              let item = coordinator.sidebarTaskItem(
                id: String(sidebarTaskReference.dropFirst("briefcal-task:".count))
              ) else {
        ...
        eventEditorError = nil
        pendingTaskTimeBlock = PendingTaskTimeBlock(
            sidebarItem: item,
            returnSection: selectedSection ?? .week
        )
        eventEditorSession = CalendarEventEditorSession(
            target: .newEvent,
            initialDraft: CalendarEventDraft(
    ...
    static func calendarEventDragReference(eventID: String) -> String {
        "briefcal-event:\(eventID)"
    }

    @discardableResult
    func beginMovingCalendarEvent(
        eventReference: String,
        to target: CalendarEventDropTarget
    ) async -> Bool {
        let prefix = "briefcal-event:"
        guard eventReference.hasPrefix(prefix),
              let event = events.first(where: {
                  $0.id == String(eventReference.dropFirst(prefix.count))
              }) else {
            eventEditorError = "The dragged calendar event is no longer available. Refresh Calendars and try again."
            return false
        }
        guard localDataOperationState == .idle else {
            eventEditorError = Self.message(
        ...
        do {
            let baseline = CalendarEventDraft(
                event: event,
                calendar: calendar
            )
            let moved = try movedEventDraft(
                baseline,
                to: target
            ).validated(
                calendar: calendar,
            ...
            guard moved != baseline else { return false }

            // Recurring events still need an explicit occurrence/series scope.
            // Ordinary events can move directly, matching native calendar drag
            // behavior without opening the full editor after every drop.
            if event.isRecurring {
                return beginEditingEvent(event, editorDraft: moved)
            }
            ...
            if case .confirmationRequired = mutationContext {
                throw CalendarEventWriteError
                    .localIdentityConfirmationRequired
            }
            let mutationImpact: EventMutationImpact?
            ...
            let preview = CalendarEventMutationPreview(
                ...
                changedFields: moved.changedFields(comparedTo: baseline)
            )
            let returnSection = selectedSection
            selectEvent(event.id)
            return await performEventMutation(
                preview,
                preserving: returnSection
            )
        } catch {
    ...
    private func movedEventDraft(
        _ draft: CalendarEventDraft,
        to target: CalendarEventDropTarget
    ) throws -> CalendarEventDraft {
        let destination: Date
        let keepsTimeOfDay: Bool
        switch target {
        case let .day(date):
            destination = calendar.startOfDay(for: date)
            keepsTimeOfDay = true
        case let .time(date):
            destination = date
            keepsTimeOfDay = false
        }

        var moved = draft
        if draft.isAllDay {
            let originalStart = calendar.startOfDay(for: draft.startDate)
            let originalEnd = calendar.startOfDay(for: draft.endDate)
            let dayCount = max(
                1,
            ...
            moved.startDate = calendar.startOfDay(for: destination)
        ...
        if keepsTimeOfDay {
            ...
            moved.startDate = destination
        }
        moved.endDate = moved.startDate.addingTimeInterval(duration)
        return moved
    }
    ...
    func beginEditingSelectedEvent() {
        ...
        _ = beginEditingEvent(event)
    }

    @discardableResult
    private func beginEditingEvent(
        _ event: DisplayEvent,
        editorDraft: CalendarEventDraft? = nil
    ) -> Bool {
        guard localDataOperationState == .idle else {
    ...
    func saveEventEditor(
        ...
        do {
            ...
            case .newEvent:
                ...
                eventEditorSession = nil
                eventEditorOperationState = .idle
                let taskBlockReturnSection = pendingTaskTimeBlock?.returnSection
                let taskBlockError = await attachPendingTaskTimeBlock(
                    to: created
                )
                await focusWrittenEvent(
                    created,
                    preserving: taskBlockReturnSection
                )
    ...
    func confirmPendingLinkedOriginalDeletion() async -> Bool {
        ...
        do {
            ...
            await finishIrreversibleLinkedDeletionFailure(
                "The original calendar event may already be deleted, but BriefCal could not verify the deletion receipt. Do not retry Delete. Your local Event Brief, notes, and tasks were kept. \(partial.localizedDescription)"
            )
            return false
        } catch {
            eventEditorError = Self.message(for: error)
    ...
    private func performEventMutation(
        _ preview: CalendarEventMutationPreview,
        preserving section: WorkspaceSection? = nil
    ) async -> Bool {
        guard eventEditorOperationState == .idle else { return false }
        eventEditorOperationState = .saving
        eventEditorError = nil
        ...
        eventEditorSession = nil
        eventEditorOperationState = .idle
        await focusWrittenEvent(receipt.event, preserving: section)
        return true
    }
    ...
    @discardableResult
    func ensureCalendarDataCovers(
        _ requestedInterval: DateInterval
    ) async -> Bool {
        guard requestedInterval.start < requestedInterval.end,
              localDataOperationState == .idle,
              calendarAuthorizationState.canReadEvents else {
            return false
        }
        if let loadedEventInterval,
           Self.interval(loadedEventInterval, covers: requestedInterval) {
            return true
        }

        let targetInterval: DateInterval
        if let loadedEventInterval,
           requestedInterval.start <= loadedEventInterval.end,
           requestedInterval.end >= loadedEventInterval.start {
            targetInterval = DateInterval(
                start: min(
                    loadedEventInterval.start,
                    requestedInterval.start
                ),
                end: max(
                    loadedEventInterval.end,
                    requestedInterval.end
                )
            )
        } else {
            targetInterval = expandedFetchInterval(
                around: requestedInterval
            )
        }
        ...
        guard let loadedEventInterval else { return false }
        return Self.interval(
            loadedEventInterval,
            covers: requestedInterval
        )
    }
    ...
    private func attachPendingTaskTimeBlock(
        ...
        do {
            let followsEventStart = EventTaskDue.relative(
                anchor: .atStart,
                offsetMinutes: 0
            )
    ...
    private func quarantineLocalData(
        ...
        let message = "\(operation) failed, and BriefCal could not restore the previous local database. Local data changes, calendar changes, and refresh are locked for this session. Quit BriefCal before recovery. The automatic pre-\(operationName) recovery ZIP is in \(automaticBackupDirectory.path(percentEncoded: false)). \(Self.message(for: error))"
        localDataQuarantineMessage = message
        setLocalDataOperationMessage(message, kind: .failure)
    ...
    private func setLocalDataOperationMessage(
        _ message: String,
        kind: LocalDataOperationMessageKind
    ) {
        localDataOperationMessage = message
        localDataOperationMessageKind = kind
    }
    ...
    private func clearSelectionOutsideVisiblePeriod() {
        ...
        let usesExtendedTimeline = selectedSection == .agenda
            || selectedSection == .month
        let selectionInterval = usesExtendedTimeline
            ? loadedEventInterval ?? visibleInterval
            : visibleInterval
        if !displayedEvents(in: selectionInterval).contains(where: {
    ...
    private func focusWrittenEvent(
        _ event: DisplayEvent,
        preserving section: WorkspaceSection? = nil
    ) async {
        let range = CalendarEventDateFormatting.effectiveDateRange(
