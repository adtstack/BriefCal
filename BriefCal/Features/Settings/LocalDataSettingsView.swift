...
private struct GeneralSettingsView: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text(
                        "Choose how calendar weeks are arranged throughout BriefCal."
                    )
                ...
                GroupBox {
                    VStack(alignment: .leading, spacing: 14) {
                        HStack(alignment: .center, spacing: 24) {
                            VStack(alignment: .leading, spacing: 3) {
                                Text("First day of week")
                                    .font(.headline)
                                Text("Used for every weekly boundary.")
                                    .font(.caption)
                                    .foregroundStyle(.secondary)
                            }
                            ...
                            .accessibilityIdentifier(
                                "settings.general.calendarWeekStart"
                            )
                        }
                        ...
                        Label(
                            "The mini month, Week, Month, and Agenda update immediately.",
                    ...
                    Label("Calendar Layout", systemImage: "calendar.day.timeline.left")
                }
            }
            .padding(24)
    ...
    private var calendarWeekStart: Binding<CalendarWeekStart> {
        Binding(
            get: { appState.calendarWeekStart },
            set: { appState.setCalendarWeekStart($0) }
        )
    }
}
            ...
            VStack(alignment: .leading, spacing: 20) {
                VStack(alignment: .leading, spacing: 6) {
                    Text("Calendars")
                        .font(.title2.weight(.semibold))
                    Text(
                        "Enable calendars for BriefCal views and Calendar Sets, and choose which ones reserve time when availability is calculated. These choices are independent."
                    )
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
                }
                ...
                if appState.calendarAccounts.isEmpty {
                    GroupBox {
                        Text(
                            "No calendars are loaded. Grant full Calendar access or refresh BriefCal, then return to this pane."
                        )
                        .foregroundStyle(.secondary)
                        .frame(maxWidth: .infinity, alignment: .leading)
                    } label: {
    ...
    private func calendarRow(_ source: CalendarSource) -> some View {
        ...
        return HStack(spacing: 12) {
            ...
            Toggle("Enabled", isOn: visibilityBinding(for: source))
                ...
                .help("Allow this calendar to appear in BriefCal and saved Calendar Sets")
            ...
            .labelsHidden()
            .frame(width: 122)
            .help("BriefCal role used by Smart Role Filters")
    ...
    private func unavailableMembershipGroup(
        ...
        return GroupBox {
            VStack(alignment: .leading, spacing: 12) {
                Text(
                    "These exact calendar identifiers are not currently available. BriefCal does not reconnect them by name."
                )
                .font(.caption)
                .foregroundStyle(.secondary)
...
private struct CalendarSetNameEditorView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            if isCreating {
                Toggle(
                    "Start with calendars currently enabled in BriefCal",
...
private extension LocalDataOperationMessageKind {
    var title: String {
        switch self {
    ...
    var backgroundColor: Color {
        switch self {
        case .success: BriefCalTheme.accentSoft
        case .failure: Color.red.opacity(0.10)
        }
    }
}
...
struct LocalDataSettingsView: View {
    ...
    @AccessibilityFocusState private var operationMessageIsFocused: Bool
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                if let message = appState.localDataOperationMessage {
                    operationMessage(
                        message,
                        kind: appState.localDataOperationMessageKind ?? .failure
                    )
                }
        ...
        .alert(
            ...
            Text(
                "BriefCal will first create an automatic backup of the current local database, then replace it with \(pendingImportURL?.lastPathComponent ?? "the selected backup"). The recovery backup remains on this Mac, and its location appears in the result. Calendar events in your connected accounts are not changed."
            )
        }
        ...
        .onAppear {
            operationMessageIsFocused = appState.localDataOperationMessage != nil
        }
        .onChange(of: appState.localDataOperationMessage) { _, message in
            operationMessageIsFocused = message != nil
        }
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("Local Data")
                .font(.title2.weight(.semibold))
            Text(
                "Back up or restore the Event Briefs, tasks, notes, and change history stored by BriefCal on this Mac."
            )
            .foregroundStyle(.secondary)
            .fixedSize(horizontal: false, vertical: true)
        }
    ...
    private var backupSection: some View {
        settingsGroup(
            ...
            Text(
                "Export a ZIP containing the BriefCal local database and a backup manifest."
            )
            .foregroundStyle(.secondary)
    ...
    private var restoreSection: some View {
        settingsGroup(
            ...
            Text(
                "Import replaces the current BriefCal local database only after validating the selected ZIP. An automatic backup of the current data is created first."
            )
            .foregroundStyle(.secondary)
    ...
    private var privacySection: some View {
        settingsGroup(
            ...
            Text(
                "Backup ZIP files are not encrypted by BriefCal. Their contents are stored in plaintext at the location you choose, including cloud folders, so keep them somewhere you trust."
            )
            .font(.caption)
            .foregroundStyle(.secondary)
            .fixedSize(horizontal: false, vertical: true)
    ...
    private var resetSection: some View {
        settingsGroup(
            ...
            Text(
                "Remove BriefCal checklists, personal tasks, notes, saved Calendar Sets, calendar preferences, and change history from the active database. An automatic recovery backup remains on this Mac, and its location appears in the result. Original calendar events remain in their calendar accounts."
            )
            .foregroundStyle(.secondary)
    ...
    private func operationMessage(
        ...
        HStack(alignment: .top, spacing: 10) {
            ...
            .frame(maxWidth: .infinity, alignment: .leading)
            .accessibilityElement(children: .combine)
            .accessibilityLabel("\(kind.title). \(message)")
            .accessibilityFocused($operationMessageIsFocused)
    ...
    private func selectExportDestination() {
        ...
        panel.title = "Export BriefCal Backup"
        panel.prompt = "Export"
    ...
    private func selectImportArchive() {
        ...
        panel.title = "Import BriefCal Backup"
    ...
    private static var suggestedBackupFilename: String {
        ...
        return "BriefCal-Backup-\(formatter.string(from: Date())).zip"
    }
}
...
private struct TaskProviderSettingsView: View {
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 20) {
                VStack(alignment: .leading, spacing: 6) {
                    Text("Task Providers")
                        .font(.title2.weight(.semibold))
                    Text(
                        "Choose where tasks created from each calendar should be mirrored. Personal tasks remain local to BriefCal in this release."
                    )
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
                }
        ...
        .confirmationDialog(
            ...
            Text(
                "This removes the saved \(provider.title) credential and account metadata from BriefCal. Remote tasks are not deleted, but linked tasks require reconnection."
            )
        }
        ...
        VStack(alignment: .leading, spacing: 12) {
            settingsGroup(title: "Apple Reminders", systemImage: "checklist") {
                HStack {
                    ...
                    Spacer()
                    appleRemindersAction
                }
                Text(appleRemindersHelpText)
                .font(.callout)
                .foregroundStyle(.secondary)
            }
    ...
    @ViewBuilder
    private var appleRemindersAction: some View {
        ...
        case .notConfigured, .notDetermined:
            ...
            .accessibilityIdentifier(
                "settings.taskProviders.requestAppleRemindersAccess"
            )
        case .authorized:
            Button("Refresh") {
                coordinator.refresh()
            ...
            .accessibilityIdentifier(
                "settings.taskProviders.refreshAppleReminders"
            )
        case .denied:
            ...
            .accessibilityIdentifier(
                "settings.taskProviders.openRemindersSettings"
            )
        case .restricted:
            Text("Restricted by policy")
                .font(.caption)
                .foregroundStyle(.secondary)
        case .unknown:
            Button("Check Again") {
            ...
            .accessibilityIdentifier(
                "settings.taskProviders.checkRemindersAccess"
            )
        }
    }

    private var appleRemindersHelpText: String {
        switch coordinator.authorizationState {
        case .notConfigured, .notDetermined, .authorized:
            "BriefCal uses the system Reminders permission and never uploads task data to a BriefCal service."
        case .denied:
            "Allow BriefCal in System Settings → Privacy & Security → Reminders, then return and refresh."
        case .restricted:
            "This Mac or your organization currently prevents apps from accessing Reminders."
        case .unknown:
            "BriefCal could not determine the Reminders permission. Check again after reviewing System Settings."
        }
    }
...
private struct ResetLocalDataConfirmationView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            Label("Reset BriefCal Local Data", systemImage: "exclamationmark.triangle.fill")
                .font(.title3.weight(.semibold))
                .foregroundStyle(.red)

            Text(
                "This removes BriefCal checklists, personal tasks, notes, role preferences, and change history from the active database. An automatic recovery backup remains on this Mac. Calendar and Exchange events are not deleted."
            )
            .fixedSize(horizontal: false, vertical: true)
