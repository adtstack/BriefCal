...
struct CalendarTimelineView: View {
    ...
    private let minimumWeekDayWidth: CGFloat = 64
    private let minimumDayWidth: CGFloat = 560
    private let minimumOverlapColumnWidth: CGFloat = 44
    private let verticalScrollerLayoutWidth = NSScroller.scrollerWidth(
        for: .regular,
    ...
    private func dayWidth(
        ...
        // Keep the whole week visible at the supported workspace width. Dense
        // overlap groups are compressed inside their own day rather than
        // widening every day in the week.
        return max(
            minimumWeekDayWidth,
    ...
    private func allDayLane(
        ...
        let dayAreaWidth = contentWidth - gutterWidth
        ...
        return ZStack(alignment: .topLeading) {
            ...
            HStack(spacing: 0) {
                ForEach(
                    ...
                    Rectangle()
                        .fill(
                            appState.calendar.isDateInToday(date)
                                ? BriefCalTheme.accentSoft.opacity(0.45)
                                : Color.clear
                        )
            ...
            .frame(width: dayAreaWidth, height: height, alignment: .topLeading)
            ...
            ScrollView(.vertical) {
                ZStack(alignment: .topLeading) {
                    ForEach(layout.allDayPlacements) { placement in
                        ...
                        .calendarTimelineEventDragSource(
            ...
            .frame(width: dayAreaWidth, height: height)
            .offset(x: gutterWidth)
    ...
    private func timelineScroller(
        ...
        ScrollViewReader { reader in
            ...
            .frame(
                width: contentWidth + verticalScrollerLayoutWidth,
    ...
    private func timelineCanvas(
        ...
        .dropDestination(for: String.self) { references, location in
            guard location.x >= gutterWidth else {
                return false
            }
            let dayIndex = Int((location.x - gutterWidth) / dayWidth)
            guard layout.dates.indices.contains(dayIndex) else {
            ...
            if let eventReference = references.first(where: {
                $0.hasPrefix("briefcal-event:")
            }) {
                Task {
                    _ = await appState.beginMovingCalendarEvent(
            ...
            guard let taskReference = references.first(where: {
                $0.hasPrefix("briefcal-task:")
            }) else {
                return false
            }
            return appState.beginCreatingTaskTimeBlock(
                sidebarTaskReference: taskReference,
                startAt: start
            )
        } isTargeted: { _ in }
        ...
        HStack(spacing: 0) {
            ...
            ForEach(Array(dates.enumerated()), id: \.element) { _, date in
                Rectangle()
                    .fill(
                        appState.calendar.isDateInToday(date)
                            ? BriefCalTheme.accentSoft.opacity(0.22)
                            : Color.clear
                    )
                    ...
                    .overlay(alignment: .center) {
                        Rectangle()
                            .fill(BriefCalTheme.subtleDivider.opacity(0.45))
                            .frame(height: 1)
                    }
                    .overlay(alignment: .leading) {
                        Rectangle()
    ...
    private func timedFrame(
        ...
        let columnSpacing = min(2, max(0.5, columnWidth * 0.08))
        let width = max(1, columnWidth - columnSpacing)
...
private struct TimedEventCard: View {
    ...
    var body: some View {
        ...
        .background(BriefCalTheme.accentSoft.opacity(0.9))
        .clipShape(RoundedRectangle(cornerRadius: 5))
        .overlay {
            RoundedRectangle(cornerRadius: 5)
                .stroke(
                    isSelected ? BriefCalTheme.accent : BriefCalTheme.subtleDivider,
...
private struct AllDayEventCard: View {
    ...
    var body: some View {
        ...
        .frame(maxWidth: .infinity, maxHeight: .infinity)
        .background(BriefCalTheme.accentSoft.opacity(0.95))
...
private struct CalendarTimelineEventDragSource: ViewModifier {
...
private extension View {
    func calendarTimelineEventDragSource(
        event: DisplayEvent,
        enabled: Bool
    ) -> some View {
        modifier(CalendarTimelineEventDragSource(
            event: event,
            enabled: enabled
        ))
    }
