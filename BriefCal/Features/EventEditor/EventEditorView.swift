...
struct EventEditorView: View {
    ...
    init(appState: AppState, session: CalendarEventEditorSession) {
        ...
        _draft = State(initialValue: session.editorDraft)
        _timeZoneInput = State(
            initialValue: session.editorDraft.timeZoneIdentifier
                ?? session.editorDraft.referenceTimeZoneIdentifier
        )
        _wantsFloatingTime = State(
            initialValue: session.editorDraft.timeZoneIdentifier == nil
        )
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            case let .unsupported(snapshot):
                Label(snapshot.summary, systemImage: "lock")
                    .font(.callout)
                Text(
                    "BriefCal preserves this complex rule without rewriting it. This occurrence may change ordinary fields; future-series and rule changes stay in Calendar.app."
                )
                .font(.caption)
                .foregroundStyle(.secondary)
            case .none, .basic:
    ...
    private var originalNotesField: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(
                "This field belongs to the Calendar.app event. Event Brief notes and tasks stay in BriefCal’s local database."
            )
            .font(.caption)
            .foregroundStyle(.secondary)
        }
    ...
    private func mutationImpactReview(
        ...
        VStack(alignment: .leading, spacing: 18) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                ForEach(
                    CalendarEventChangedField.allCases.filter {
                        change.changedFields.contains($0)
                    },
                    id: \.self
                ) { field in
                    let values = impactValues(for: field, change: change)
                    impactRow(
                        title: values.title,
    ...
    private func impactValues(
        for field: CalendarEventChangedField,
        change: CalendarEventMutationPreview
    ) -> (title: String, before: String, after: String) {
        switch field {
        case .title:
            return (
                "Title",
                displayImpactValue(change.original.title),
                displayImpactValue(change.draft.title)
            )
        case .calendar:
            return (
                "Calendar",
                calendarLabel(for: change.original),
                calendarLabel(identifier: change.draft.calendarIdentifier)
            )
        case .time:
            return (
                "Time",
        ...
        case .location:
            return (
                "Location",
                displayImpactValue(change.original.location),
                displayImpactValue(change.draft.location)
            )
        case .originalNotes:
            return (
                "Original event notes",
                displayImpactValue(change.original.originalNotes),
                displayImpactValue(change.draft.originalNotes)
            )
        case .recurrence:
            return (
                "Repeat",
                change.original.recurrence.editorSummary,
                change.draft.recurrence.editorSummary
            )
        case .deletion:
            return (
                "Deletion",
                "Original calendar event exists",
                "Original calendar event will be removed"
            )
        }
    }

    private func displayImpactValue(_ value: String?) -> String {
    ...
    private var scopeHelpText: String {
        ...
        case .none:
            return "Choose a scope before Save. BriefCal does not select a series-wide default."
        case .thisEvent:
