...
struct EventBriefView: View {
    ...
    @State private var showsTaskSections = true
    ...
    private var localEditingBadge: some View {
        Label("Event Brief notes · On this Mac", systemImage: "macbook")
            ...
            .padding(.vertical, 4)
            .background(BriefCalTheme.accentSoft, in: Capsule())
    ...
    private var taskDestinationBadge: some View {
        if let destination = appState.taskDestinationSummary(
            ...
            .font(.caption.weight(.medium))
            .foregroundStyle(
                isAuthorized ? Color.primary : Color.orange
            )
            .lineLimit(2)
            .padding(.horizontal, 8)
            .padding(.vertical, 4)
            ...
            Label("New tasks · Local only", systemImage: "macbook")
                .font(.caption.weight(.medium))
                .foregroundStyle(.secondary)
                .padding(.horizontal, 8)
                .padding(.vertical, 4)
    ...
    private func identityBlockedContent(
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            .font(.headline)
            Text(
                "Editing is paused so BriefCal does not attach local notes or tasks "
    ...
    private func editableContent(
        ...
        VStack(alignment: .leading, spacing: 18) {
            ...
            DisclosureGroup(isExpanded: $showsTaskSections) {
                VStack(alignment: .leading, spacing: 18) {
                ...
                Label(
                    tasks.isEmpty ? "Tasks" : "Tasks (\(tasks.count))",
                    systemImage: "checklist"
                )
                .font(.headline)
            }
            .accessibilityIdentifier("eventBrief.tasks")
            .onAppear {
                if tasks.isEmpty {
                    showsTaskSections = false
                }
            }
            .onChange(of: tasks.isEmpty) { wasEmpty, isEmpty in
                if wasEmpty && !isEmpty {
                    showsTaskSections = true
                }
            }
    ...
    private var cancelledEventBanner: some View {
        VStack(alignment: .leading, spacing: 5) {
            Label("Calendar event cancelled", systemImage: "calendar.badge.exclamationmark")
                .font(.headline)
            Text(
                "The calendar provider reported this status. Local notes and tasks were kept; BriefCal did not delete or change the calendar event."
            )
            .font(.caption)
            .foregroundStyle(.secondary)
        }
    ...
    private func endedEventBanner(hasStoredBrief: Bool) -> some View {
        ...
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(
            BriefCalTheme.accentSoft.opacity(0.55),
            in: RoundedRectangle(cornerRadius: 10)
        )
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Label("References", systemImage: "link")
                .font(.headline)
            Text("Links stay local to this Event Brief. BriefCal does not copy remote page content.")
                .font(.caption)
                .foregroundStyle(.secondary)
...
private struct EventTaskRow: View {
    ...
    var body: some View {
        HStack(alignment: .firstTextBaseline, spacing: 8) {
            ...
            .buttonStyle(.plain)
            .foregroundStyle(task.isCompleted ? BriefCalTheme.accent : .secondary)
