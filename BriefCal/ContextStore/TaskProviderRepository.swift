...
final class TaskProviderRepository {
    ...
    /// for Microsoft To Do tasks that are not linked to a BriefCal Event Brief.
    @discardableResult
        ...
        return try database.write { db in
            ...
            if let destinationItem, destinationItem.id != sourceItem.id {
                if let owner = try TaskBindingRecord
                    ...
                    throw TaskProviderError.providerFailure(
                        "The moved remote task is already linked to a different BriefCal task."
                    )
                }
                try db.execute(
                    sql: """
        ...
        return try database.write { db in
            ...
            if let existingBinding = try TaskBindingRecord
                ...
                guard existingBinding.eventTaskID == eventTaskID else {
                    throw TaskProviderError.providerFailure(
                        "The remote task is already linked to a different BriefCal task."
                    )
                }
    ...
    func replaceLinkedTask(
        ...
        return try database.write { db in
            if applyRemoteToEventTask {
                ...
                let due = eventTaskDueOverride
                    ?? remote.dueAt.map(EventTaskDue.fixed)
                    ?? .none
            ...
            if let owner = try TaskBindingRecord
                ...
                throw TaskProviderError.providerFailure(
                    "The selected remote task is already linked to a different BriefCal task."
                )
            }
