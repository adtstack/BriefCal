...
struct DatabaseBackupManifest: Codable, Equatable, Sendable {
    ...
    static let applicationIdentifier = "com.adtstack.briefcal"
    static let databaseFilename = "briefcal.sqlite"
...
/// Owns BriefCal-local data backup, restore, and reset. It never reads from or
/// writes to EventKit.
struct LocalDataBackupService: Sendable {
    ...
    func exportBackup(
        ...
        return try withTemporaryDirectory(prefix: "BriefCal-Export") { directory in
    ...
    func importBackup(
        ...
        try withPreparedBackup(at: archiveURL) { manifest, importedDatabaseURL in
            try withTemporaryDirectory(prefix: "BriefCal-Import-Rollback") { directory in
    ...
    func resetLocalData(
        ...
        try withTemporaryDirectory(prefix: "BriefCal-Reset-Rollback") { directory in
    ...
    private func deleteAllLocalData() throws -> LocalDataDeletedRowCounts {
        try database.write { db in
            ...
            // Keep GRDB's migration ledger, and remove only BriefCal-owned user
    ...
    private func withPreparedBackup<T>(
        ...
        guard entries.count == 2,
            ...
            throw LocalDataBackupError.invalidArchive(
                "exactly manifest.json and briefcal.sqlite are required"
            )
        }
        guard manifestData.count <= Self.maximumManifestByteCount else {
            throw LocalDataBackupError.fileTooLarge(
        ...
        return try withTemporaryDirectory(prefix: "BriefCal-Import-Preflight") { directory in
    ...
    private func makeAutomaticBackupURL(
        ...
        return directory.appendingPathComponent(
            "BriefCal-\(operation)-\(timestamp)-\(token).zip"
        )
    }
    ...
    var errorDescription: String? {
        ...
        case let .recoveryFailed(reason, rollbackSucceeded):
            rollbackSucceeded
                ...
                : "Recovery did not finish, and BriefCal could not put every original database file back: \(reason)"
        }
    }
}

/// Restores a strictly validated BriefCal backup when the normal application
...
struct BootstrapLocalDataRecoveryService: Sendable {
    ...
    func recover(
        ...
        do {
            ...
            let note = """
                BriefCal preserved this SQLite file family after a failed application bootstrap.
...
private enum DatabaseSchemaValidator {
    ...
    static func validateDatabase(at fileURL: URL) throws -> DatabaseSchemaInspection {
        ...
        configuration.label = "BriefCal Backup Preflight"
    ...
    private static func requireCurrentSchema(
        ...
        guard actual.schemaObjects == expected.schemaObjects else {
            throw LocalDataBackupError.invalidDatabase(
                "sqlite_master does not exactly match the current BriefCal schema"
            )
        }
    }
}
...
private enum StrictZIPArchive {
    ...
    static func encode(entries: [Entry]) throws -> Data {
        guard entries.map(\.name) == ["manifest.json", "briefcal.sqlite"] else {
