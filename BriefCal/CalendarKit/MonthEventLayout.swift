...
struct MonthGrid: Equatable {
    ...
    init(
        ...
        let normalizedMinimumWeekCount = min(6, max(4, minimumWeekCount))
        let weekCount = min(
            6,
            max(normalizedMinimumWeekCount, (daysThroughMonthEnd + 6) / 7)
        )
    ...
    init(
        startingAtWeekContaining date: Date,
        weekCount: Int,
        calendar: Calendar
    ) {
        let resolvedWeekStart = calendar.dateInterval(
            of: .weekOfYear,
            for: date
        )?.start ?? calendar.startOfDay(for: date)
        let normalizedWeekCount = max(1, weekCount)
        let dayCount = normalizedWeekCount * 7
        let gridEnd = calendar.date(
            byAdding: .day,
            value: dayCount,
            to: resolvedWeekStart
        ) ?? resolvedWeekStart.addingTimeInterval(
            Double(dayCount) * 86_400
        )
...
enum MonthTimeline {
    ...
    static func anchorWeekStart(
        containing date: Date,
        calendar: Calendar
    ) -> Date {
        let monthStart = MonthGrid.monthStart(
            containing: date,
            calendar: calendar
        )
    ...
    static func grid(
        ...
        MonthGrid(
            startingAtWeekContaining: anchorWeekStart(
                containing: date,
                calendar: calendar
            ),
            weekCount: weekCount,
    ...
    static func identifier(
        ...
        MonthGrid.dayIdentifier(for: date, calendar: calendar)
    }
}
