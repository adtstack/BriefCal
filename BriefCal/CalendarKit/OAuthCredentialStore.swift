...
enum OAuthCredentialStoreError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .keychain:
            "BriefCal could not access the secure credential store."
        case .invalidCredential:
...
final class KeychainOAuthCredentialStore: OAuthCredentialStoring {
    ...
    init(
        service: String = Bundle.main.bundleIdentifier
            ...
            ?? "com.briefcal.task-provider-oauth"
    ) {
...
struct OAuthProviderConfiguration: Equatable {
    ...
    static func load(
        ...
        switch provider {
        case .googleTasks:
            prefix = "BriefCalGoogleTasks"
        case .todoist:
            prefix = "BriefCalTodoist"
        case .microsoftToDo:
            prefix = "BriefCalMicrosoftToDo"
        case .appleReminders:
            return nil
        }
        ...
        let clientSecret = provider == .googleTasks
            ? validString(bundle.object(
                forInfoDictionaryKey: "BriefCalGoogleTasksClientSecret"
            ))
            : nil
...
enum OAuthLoopbackAuthorizationError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .listenerUnavailable:
            "BriefCal could not start the local OAuth callback listener."
        case .browserDidNotOpen:
            "BriefCal could not open the system browser for sign-in."
        case .malformedCallback:
...
enum OAuthLoopbackBrowserAuthorization {
    ...
    static func callbackHTTPResponse(succeeded: Bool) -> String {
        if succeeded {
            return "HTTP/1.1 200 OK\r\nContent-Type: text/plain; charset=utf-8\r\nConnection: close\r\n\r\nSign-in response received. Return to BriefCal while it finishes connecting."
        }
        return "HTTP/1.1 400 Bad Request\r\nContent-Type: text/plain; charset=utf-8\r\nConnection: close\r\n\r\nSign-in was not completed. Return to BriefCal to see the error and try again."
    }
