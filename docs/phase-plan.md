...
- Phase 7: **7A~7B 구현·전체 자동·Release checkpoint 완료 / 7C 대기** — active occurrence의 종료 시각에서 scheduled/completed를 파생하고 After Review와 후속 작업 projection을 구현했다. 전용 occurrence-aware lookup, 두 단계 missing 확인, orphan 보관, 명시적 재연결과 local-only 삭제까지 보강한 현재 suite는 175 tests 중 1 intentional opt-in skip, 0 failures다. KaosCal에서 linked 원본을 삭제하는 review는 Phase 7C까지 잠겨 있다.
...
- Phase 6의 **121-test 구현 checkpoint**, 122-test read-only gate와 132-test legacy compatibility checkpoint, Phase 7A의 145-test checkpoint, mini month·AppIcon의 154-test checkpoint를 보존한다. Phase 7B 복구와 최종 안전 회귀를 추가한 현재 suite는 **175 tests, 1 intentional opt-in skip, 0 failures, 0 unexpected**다. 최종 result bundle은 `/private/tmp/KaosCalPhase7BFinal-20260712-0155.xcresult`다.
...
- [ADR-012](adr/ADR-012-lifecycle-after-review-and-orphan-confirmation.md)에서 7A와 7B의 안전 경계를 확정했다.
