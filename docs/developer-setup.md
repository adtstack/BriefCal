...
5. **앱 권한 — 사용자 허용 보고 / 최신 build 확인 대기**
   - 사용자는 2026-07-11 macOS 전체 캘린더 접근을 허용했다고 보고했다.
   - 이 보고를 최신 ad-hoc binary의 화면 pass로 간주하지 않는다. 실행 중인 검증 대상 build에서 `Full calendar access`와 실제 EventKit fetch를 확인한다.
...
3. Day/Week/Agenda에 같은 테스트 일정이 표시된다.
4. 민감하지 않은 `KC-E1`을 KaosCal에서 생성한 뒤 Calendar.app에서 확인한다.
