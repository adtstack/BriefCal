...
BriefCal에는 모든 캘린더 관점과 눈에 보이는 할 일 목록이 필요하다. 기존 문서는 Event Brief 중심이었지만, 빠른 개인 작업의 위치가 없었다.

## 결정

- Day, Week, Agenda는 모두 v1 필수 화면이다.
- 세 화면은 공통 `DisplayEvent`와 시간 의미 모델을 사용한다.
- Agenda는 선택한 날짜가 속한 주를 시작점으로 삼고, 이후 주를 시간순으로 이어 붙이는
  연속 목록이다. 각 주는 옅은 날짜 범위로만 구분하고 아래로 scroll하면 다음 주를 필요한
  만큼 추가한다. 일정이 없는 주는 별도 안내 문구 없이 짧은 빈 공간만 유지한다.
- Agenda는 선택 주부터 앞으로 탐색하는 흐름이며 모든 과거·미래 일정을 한 번에 불러온다는
  의미는 아니다. 날짜 이동은 목록의 시작 주를 바꾸고 전체 일정 검색은 별도 검색/Upcoming
  기능으로 다룬다.
...
- BusyCal의 정보 밀도와 3-pane 작업 흐름만 참고한다. BriefCal의 색·타이포그래피·아이콘·레이아웃은 독자적으로 만든다.
