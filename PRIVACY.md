...
BriefCal은 계정 가입 없이 작동하는 macOS 앱이다. 원본 일정은 macOS EventKit을 통해
사용자가 이미 구성한 Calendar 계정에 저장된다. BriefCal의 Event Brief, 작업, 로컬
notes, 연결 정보, calendar role·usage, saved Calendar Set과 변경 기록은 이 Mac의 로컬
SQLite에 저장된다.

현재 앱 코드에는 BriefCal 서버, 자체 계정 시스템, AI SDK/API, 분석/광고 SDK 또는
...
Todoist 또는 Microsoft To Do provider와 직접 통신할 수 있지만 BriefCal 중계 서버는
사용하지 않는다. 사용자가 backup을 cloud-mounted 폴더에 저장하면 해당 파일은 macOS와
그 위치의 제공자에 의해 전송될 수 있으며, 이는 BriefCal Cloud 기능이 아니다.

## BriefCal이 접근하는 데이터

### macOS Calendar 데이터

사용자가 `Full Calendar Access`를 허용하면 BriefCal은 EventKit에서 다음과 같은 정보를
...
BriefCal은 사용자 요청 없이 RSVP하거나 참석자 목록을 변경하지 않는다. 지원되는 새
...
권한은 macOS System Settings에서 거부하거나 취소할 수 있다. 권한 취소는 BriefCal의
기존 로컬 DB나 사용자가 만든 backup을 자동 삭제하지 않는다.

### Task provider 데이터

사용자가 Settings에서 provider account와 destination을 명시적으로 연결하면 BriefCal은
...
일반 provider task의 notes는 상세 조회·편집 중 메모리에서만 사용하고 BriefCal SQLite나
backup에 복제하지 않는다. Provider를 연결하지 않으면 이 직접 통신은 필요하지 않고
local-only 기능은 계속 동작한다.

### BriefCal 로컬 데이터

다음 정보가 `briefcal.sqlite`에 저장될 수 있다.
...
BriefCal은 Exchange/task-provider password, MFA code, tenant/client secret을 앱 입력으로
...
`BriefCal/briefcal.sqlite`를 사용한다. 실제 경로는 앱의 **Settings > Local Data >
...
BriefCal은 App Sandbox와 macOS Calendar 권한을 사용하지만, active SQLite에 별도의
...
- ZIP과 SQLite는 BriefCal이 암호화하거나 제작자 서명하지 않는 plaintext다.
...
- 사용자가 고른 로컬·외장·network-mounted·cloud 위치에 수동 ZIP을 저장할 수 있다.
  ...
  정책에 따른다.
- Import/reset 전 자동 recovery ZIP은 active DB 옆 `BriefCal/Backups`에 저장된다.
...
BriefCal은 AI, telemetry, 광고, 분석, BriefCal 계정 또는 BriefCal Cloud endpoint로 사용자
데이터를 전송하지 않는다. 다음 처리는 사용자의 설정과 명시적 동작에 따라 BriefCal 외부에서
일어날 수 있다.
...
- 서명된 direct-download build가 구성된 경우, 새 버전을 확인하고 받기 위한 Sparkle의
  정적 HTTPS appcast·archive 요청. BriefCal은 이 요청에 Calendar/Event Brief/task/backup
...
BriefCal은 이 외부 서비스의 계정, 보존, 공유 또는 삭제 정책을 제어하지 않는다.
...
BriefCal은 서버 계정이나 서버 측 Event Brief 저장소를 운영하지 않으므로 현재
...
AI, BriefCal 서버·계정·Cloud, telemetry와 remote analytics는
[ADR-019](docs/adr/ADR-019-local-only-no-ai-no-product-cloud.md)에 따라 제품 범위에서 영구
제외한다. 자동업데이트의 최소 경계는
