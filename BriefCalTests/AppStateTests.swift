...
final class AppStateTests: XCTestCase {
    ...
    func testUITestBootstrapUsesIsolatedFixtureBeforeHostedTestGuard() async throws {
        ...
        state.userSelectEvent("ui-event-planning")
        state.rightSidebarMode = .tasks
        state.userSelectEvent("ui-event-planning")

        XCTAssertEqual(state.rightSidebarMode, .eventDetails)
    }

    func testUITestRefreshFailurePreservesLoadedFixture() async {
        let state = AppBootstrap.makeAppState(
            environment: ["BRIEFCAL_UI_TEST_SCENARIO": "refresh-failure"],
            arguments: ["BriefCal", "--ui-testing"]
        )
    ...
    func testBootstrapRecoveryIsOfferedOnlyWhenNoLiveDatabaseWriterExists() throws {
        ...
        let databaseURL = directory.appendingPathComponent("briefcal.sqlite")
    ...
    func testCalendarWeekStartUpdatesEveryCalendarBoundaryAndPersists() throws {
        let suiteName = "BriefCalTests.CalendarWeekStart.\(UUID().uuidString)"
        ...
        calendar.firstWeekday = CalendarWeekStart.sunday.firstWeekday
        let focusedDate = try XCTUnwrap(calendar.date(from: DateComponents(
            year: 2026,
            month: 7,
            day: 8,
            hour: 12
        )))
        let sundayWeekStart = try XCTUnwrap(calendar.date(from: DateComponents(
            year: 2026,
            month: 7,
            day: 5
        )))
        let mondayWeekStart = try XCTUnwrap(calendar.date(from: DateComponents(
            year: 2026,
            month: 7,
            day: 6
        )))
        ...
        XCTAssertEqual(state.calendarWeekStart, .sunday)
        XCTAssertEqual(state.calendar.firstWeekday, 1)
        XCTAssertEqual(state.visibleDates.first, sundayWeekStart)
        let originalFocusedDay = state.focusedDate
        XCTAssertEqual(
            MonthGrid(
                containing: focusedDate,
                calendar: state.calendar
            ).weekdayOrdinals.first,
            1
        )

        state.setCalendarWeekStart(.monday)

        XCTAssertEqual(state.calendarWeekStart, .monday)
        XCTAssertEqual(state.calendar.firstWeekday, 2)
        XCTAssertEqual(state.focusedDate, originalFocusedDay)
        XCTAssertEqual(state.visibleDates.first, mondayWeekStart)
        XCTAssertEqual(
            MonthGrid(
                containing: focusedDate,
                calendar: state.calendar
        ...
        XCTAssertEqual(reopenedState.calendarWeekStart, .monday)
        XCTAssertEqual(reopenedState.calendar.firstWeekday, 2)
        XCTAssertEqual(reopenedState.visibleDates.first, mondayWeekStart)
    }
    ...
    func testMonthTimelineUsesContinuousCivilWeeksAndLocalizedBoundaries() throws {
        var calendar = Calendar(identifier: .gregorian)
        ...
        calendar.firstWeekday = 2
        let focus = try XCTUnwrap(calendar.date(from: DateComponents(
            year: 2026,
            month: 3,
            day: 15,
            hour: 12
        ...
        XCTAssertEqual(grid.rowCount, MonthTimeline.initialWeekCount)
        XCTAssertEqual(grid.days.count, MonthTimeline.initialWeekCount * 7)
        XCTAssertEqual(Set(grid.days.map(\.id)).count, grid.days.count)
        XCTAssertTrue(grid.days.allSatisfy(\.isInDisplayedMonth))
        XCTAssertEqual(
            calendar.dateComponents(
                [.day],
                from: grid.visibleInterval.start,
                to: grid.visibleInterval.end
            ).day,
            MonthTimeline.initialWeekCount * 7
        )
        XCTAssertNotEqual(
            grid.visibleInterval.duration,
            Double(MonthTimeline.initialWeekCount * 7 * 86_400)
        )

        let julyFirst = try XCTUnwrap(calendar.date(from: DateComponents(
            year: 2026,
            month: 7,
            day: 1
        )))
        XCTAssertTrue(MonthTimeline.isMonthStart(julyFirst, calendar: calendar))
        XCTAssertEqual(
            CalendarEventDateFormatting.monthName(
                julyFirst,
                calendar: calendar
            ),
            "7월"
    ...
    func testAgendaTimelineUsesContinuousCivilWeeksAcrossDST() throws {
        var calendar = Calendar(identifier: .gregorian)
        calendar.timeZone = try XCTUnwrap(
            TimeZone(identifier: "America/New_York")
        )
        ...
        let weeks = AgendaTimeline.weeks(
            startingAt: focus,
            count: 3,
            calendar: calendar
        )
        ...
        XCTAssertEqual(weeks[0].endDate, weeks[1].startDate)
        XCTAssertEqual(weeks[1].endDate, weeks[2].startDate)
        XCTAssertEqual(
            calendar.dateComponents(
                [.day],
        ...
        XCTAssertNotEqual(
            weeks[0].endDate.timeIntervalSince(weeks[0].startDate),
            7 * 86_400
        )
        XCTAssertEqual(
            AgendaTimeline.identifier(for: weeks[0], calendar: calendar),
            "2026-03-02"
        )
    }

    func testAgendaRangeLoadingExpandsCoverageOnlyWhenNeeded() async throws {
        var calendar = Calendar(identifier: .gregorian)
        calendar.timeZone = TimeZone(secondsFromGMT: 0)!
        calendar.locale = Locale(identifier: "en_US_POSIX")
        let baseline = try XCTUnwrap(calendar.date(from: DateComponents(
        ...
        await state.refreshCalendarData()
        let initialInterval = try XCTUnwrap(provider.lastFetchInterval)
        let requestedEnd = try XCTUnwrap(calendar.date(
            byAdding: .day,
            value: 168,
            to: baseline
        ))
        let requestedInterval = DateInterval(
            start: AgendaTimeline.weekStart(
                containing: baseline,
                calendar: calendar
            ),
            end: requestedEnd
        ...
        let didExpandCoverage = await state.ensureCalendarDataCovers(
            requestedInterval
        )
        XCTAssertTrue(didExpandCoverage)
        XCTAssertEqual(provider.fetchCallCount, 2)
        XCTAssertEqual(
            provider.lastFetchInterval,
            DateInterval(
                start: min(initialInterval.start, requestedInterval.start),
                end: max(initialInterval.end, requestedInterval.end)
            )
        )

        let didReuseCoverage = await state.ensureCalendarDataCovers(
            requestedInterval
        )
        XCTAssertTrue(didReuseCoverage)
        XCTAssertEqual(provider.fetchCallCount, 2)
    }
...
final class Phase6AppStateTests: XCTestCase {
    func testDraggingProviderTaskCreatesSnappedTimeBlockAndLinkedDuringTask() async throws {
        ...
        XCTAssertTrue(state.beginCreatingTaskTimeBlock(
            sidebarTaskReference: "briefcal-task:\(sidebarID)",
        ...
        XCTAssertEqual(
            eventTask.due,
            .relative(anchor: .atStart, offsetMinutes: 0)
        )
        XCTAssertEqual(taskProvider.updateTaskCount, 1)
        XCTAssertEqual(
            taskProvider.snapshots.first(where: { $0.id == remote.id })?.dueAt,
            date(2026, 7, 10, 10)
        )
        ...
        let didMove = await state.beginMovingCalendarEvent(
            eventReference: AppState.calendarEventDragReference(
                eventID: createdEvent.id
            ),
            to: .time(date(2026, 7, 10, 13, 30))
        )
        XCTAssertTrue(didMove)
        XCTAssertNil(state.eventEditorSession)
        XCTAssertNil(state.pendingEventMutation)
        XCTAssertEqual(state.selectedSection, .week)
        XCTAssertEqual(taskProvider.updateTaskCount, 2)
        XCTAssertEqual(
            taskProvider.snapshots.first(where: { $0.id == remote.id })?.dueAt,
            date(2026, 7, 10, 13, 30)
        )
    }

    func testDraggingTimedEventMovesImmediatelyAndPreservesDuration() async throws {
        let event = makeEvent(
            id: "drag-timed",
            start: date(2026, 7, 10, 10),
            end: date(2026, 7, 10, 11, 30)
        )
        ...
        XCTAssertEqual(
            provider.lastUpdatedDraft?.startDate,
            date(2026, 7, 11, 14, 15)
        )
        XCTAssertEqual(
            provider.lastUpdatedDraft?.endDate,
            date(2026, 7, 11, 15, 45)
        )
    }

    func testDraggingRecurringEventStillRequestsMutationScope() async throws {
        let event = makeEvent(
            id: "drag-recurring",
            ...
            start: date(2026, 7, 10, 10),
            end: date(2026, 7, 10, 11)
        )
        let provider = makeProvider(events: [event])
        let state = makeState(provider: provider)
        await state.loadCalendarStatus()

        let didPrepareMove = await state.beginMovingCalendarEvent(
            eventReference: AppState.calendarEventDragReference(
                eventID: event.id
            ),
            to: .time(date(2026, 7, 11, 14, 15))
        ...
        XCTAssertTrue(didPrepareMove)
        let session = try XCTUnwrap(state.eventEditorSession)
        XCTAssertEqual(
            session.editorDraft.startDate,
            date(2026, 7, 11, 14, 15)
        )
        XCTAssertEqual(
            session.editorDraft.endDate,
            date(2026, 7, 11, 15, 15)
        )
        XCTAssertEqual(provider.updateCallCount, 0)
    }

    func testDraggingEventToMonthDayPreservesTimeAndRejectsReadOnlyCalendar() async throws {
        let event = makeEvent(
            id: "drag-month",
            start: date(2026, 7, 10, 10, 30),
        ...
        let didMove = await state.beginMovingCalendarEvent(
            ...
            to: .day(date(2026, 7, 15))
        )
        XCTAssertTrue(didMove)
        XCTAssertEqual(
            provider.lastUpdatedDraft?.startDate,
            date(2026, 7, 15, 10, 30)
        )
        XCTAssertEqual(
            provider.lastUpdatedDraft?.endDate,
            date(2026, 7, 15, 12)
        ...
        let readOnly = makeEvent(
            id: "drag-read-only",
            start: date(2026, 7, 12, 9),
            isReadOnly: true
        )
        provider.events = [readOnly]
        await state.reloadCalendarData()

        let didMoveReadOnlyEvent = await state.beginMovingCalendarEvent(
            eventReference: AppState.calendarEventDragReference(
                eventID: readOnly.id
            ),
            to: .day(date(2026, 7, 16))
        )
        XCTAssertFalse(didMoveReadOnlyEvent)
        XCTAssertNil(state.eventEditorSession)
    ...
    func testTaskDeepLinkUsesLocalBindingThenStrongCalendarLookup() async throws {
        ...
        await state.openTaskDeepLink(
            try XCTUnwrap(URL(string: "briefcal://task/\(task.id)"))
        )
    ...
    func testActiveCalendarSetSourcesShowOnlySavedMembersOrMatchingRoles() async throws {
        ...
        XCTAssertTrue(state.setCalendarRole(.work, for: work))
        XCTAssertTrue(state.setCalendarRole(.personal, for: personal))

        let savedSet = try XCTUnwrap(state.createCalendarSet(
            name: "Personal Focus",
            calendarIdentifiers: [personal.id]
        ))
        XCTAssertTrue(state.selectCalendarSet(.saved(savedSet.id)))
        XCTAssertEqual(
            state.selectedCalendarSetSources.map(\.id),
            [personal.id]
        )

        XCTAssertTrue(state.setCalendarVisibility(false, for: personal))
        XCTAssertEqual(
            state.selectedCalendarSetSources.map(\.id),
            [personal.id],
            "Disabled members remain visible in the sidebar so they can be re-enabled"
        )

        XCTAssertTrue(state.selectCalendarSet(.role(.work)))
        XCTAssertEqual(state.selectedCalendarSetSources.map(\.id), [work.id])
        XCTAssertTrue(state.selectCalendarSet(.all))
        XCTAssertEqual(
            state.selectedCalendarSetSources.map(\.id),
            state.calendarSources.map(\.id)
        )
    }

    func testActiveCalendarSetSidebarFitsAndProducesReadableMemberSnapshot() async throws {
        let provider = makeProvider(events: [])
        let store = ContextStore(database: try AppDatabase.inMemory())
        let state = makeState(provider: provider, store: store)
        await state.loadCalendarStatus()
        ...
        if let snapshotPath = ProcessInfo.processInfo.environment[
            "BRIEFCAL_SIDEBAR_SNAPSHOT_PATH"
        ] {
        ...
        attachment.name = "Active Calendar Set Sidebar"
    ...
    func testSavedCalendarSetVisibilityUsesGlobalEnableAndMembershipWhileBlockingStaysIndependent() async throws {
        ...
        XCTAssertEqual(
            ...
            "All calendars in Member Only are disabled in BriefCal"
        )
        XCTAssertEqual(
            state.blockingEvents.map(\.id),
            [member.id, nonmember.id]
    ...
    func testMonthTimelineFitsMinimumWorkspaceAndProducesOffscreenBitmap() async throws {
        let events = [
            makeEvent(
                id: "month-all-day",
        ...
        func containsScrollView(_ view: NSView) -> Bool {
            view is NSScrollView
                || view.subviews.contains(where: containsScrollView)
        }
        XCTAssertTrue(
            containsScrollView(hostingView),
            "Month view should continue into later months in a scroll view."
        )
        ...
        attachment.name = "BriefCal Month Timeline"
        attachment.lifetime = .keepAlways
        add(attachment)
        if let snapshotPath = ProcessInfo.processInfo.environment[
            "BRIEFCAL_MONTH_SNAPSHOT_PATH"
        ] {
            try pngData.write(to: URL(fileURLWithPath: snapshotPath))
        }
        window.contentView = nil
    ...
    func testTimelineAllDayAndTimedGridProduceAlignedOffscreenBitmap() async throws {
        let events = [
            makeEvent(
                id: "timeline-all-day",
            ...
            makeEvent(
                ...
                start: date(2026, 8, 3, 9),
                end: date(2026, 8, 3, 10)
            )
        ]
        let provider = makeProvider(events: events)
        let focusedDate = date(2026, 8, 3, 9)
        let state = AppState(
            calendar: calendar,
            now: { focusedDate },
            calendarProvider: provider
        ...
        let hostingView = NSHostingView(rootView:
            CalendarTimelineView(appState: state)
                .frame(width: width, height: height)
                .background(Color(nsColor: .windowBackgroundColor))
                .environment(\.colorScheme, .light)
        )
        ...
        XCTAssertGreaterThan(pngData.count, 8_000)
        let rendered = try XCTUnwrap(NSBitmapImageRep(data: pngData))
        let scale = CGFloat(rendered.pixelsWide) / width
        func dividerColumn(atTopY topY: CGFloat) -> Int? {
            let pixelY = rendered.pixelsHigh - 1 - Int(topY * scale)
            let startX = Int(56 * scale)
            let endX = Int(72 * scale)
            return (startX...endX).first { pixelX in
                guard let pixelColor = rendered.colorAt(
                    x: pixelX,
                    y: pixelY
                ), let color = pixelColor.usingColorSpace(.deviceRGB) else {
                    return false
                }
                return color.redComponent < 0.97
                    && color.greenComponent < 0.97
                    && color.blueComponent < 0.97
            }
        }
        XCTAssertEqual(
            dividerColumn(atTopY: 72),
            dividerColumn(atTopY: 112),
            "All-day and timed grids must share the same left divider column"
        )
        let attachment = XCTAttachment(
            data: pngData,
            uniformTypeIdentifier: "public.png"
        )
        attachment.name = "BriefCal Timeline Alignment"
        attachment.lifetime = .keepAlways
        add(attachment)
        window.contentView = nil
    }
...
final class Phase9AppStateTests: XCTestCase {
    func testExportFlushesPendingSelectedNotesWithoutCalendarWrites() async throws {
        ...
        XCTAssertEqual(state.localDataOperationMessageKind, .success)
        XCTAssertNotNil(state.localDataOperationMessage)
    ...
    func testImportRollbackFailureQuarantinesMutationsAndRefreshForSession() async throws {
        ...
        XCTAssertEqual(state.localDataOperationMessageKind, .failure)
        ...
        XCTAssertTrue(message.contains("Quit BriefCal before recovery"))
    ...
    func testGeneralSettingsFitsAndProducesOffscreenBitmap() throws {
        let state = AppState(calendarProvider: FakeCalendarProvider())
        XCTAssertEqual(state.selectedSettingsPane, .general)
    ...
    func testCalendarSetSettingsFitsAndProducesOffscreenBitmap() async throws {
        ...
        try autoreleasepool {
            ...
            XCTAssertGreaterThan(pngData.count, 10_000)
            if let snapshotPath = ProcessInfo.processInfo.environment[
                "BRIEFCAL_CALENDAR_SET_SNAPSHOT_PATH"
            ] {
                try pngData.write(to: URL(fileURLWithPath: snapshotPath))
            }
            window.contentView = nil
    ...
    private func assertCalendarUsageSettingsBitmap(
        ...
        try autoreleasepool {
            ...
            state.selectedSettingsPane = .calendars
            let hostingView = NSHostingView(rootView:
                SettingsRootView(appState: state)
                    .background(Color(nsColor: .windowBackgroundColor))
            )
    ...
    func testBootstrapRecoveryStateFitsAndProducesOffscreenBitmap() throws {
        let longStartupError = Array(
            repeating: "database disk image is malformed while validating the preserved SQLite sidecar",
            count: 8
        ).joined(separator: ". ")
        let longRecoveryError = Array(
            repeating: "The selected archive could not be restored because validation did not complete",
            count: 4
        ).joined(separator: ". ")
        let hostingView = NSHostingView(rootView:
            BootstrapLocalDataRecoveryView(
                message: longStartupError,
                operationState: .failed(longRecoveryError),
                databaseURL: URL(
                    fileURLWithPath: "/private/example/BriefCal/Preserved Startup Data/very-long-account-and-workspace-name/briefcal.sqlite"
                ),
                recover: { _ in }
            )
        ...
        attachment.name = "BriefCal Bootstrap Recovery Long Error"
        attachment.lifetime = .keepAlways
        add(attachment)
    }
