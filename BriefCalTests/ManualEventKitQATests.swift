...
final class ManualEventKitQATests: XCTestCase {
    func testManualExchangeGate() async throws {
        ...
        guard environment["BRIEFCAL_EVENTKIT_QA_MODE"] == "inspect" else {
            throw XCTSkip(
                "Manual EventKit QA is read-only and opt-in. "
                    + "Set BRIEFCAL_EVENTKIT_QA_MODE=inspect (writes disabled)."
            )
        }
        let mode = "inspect"
        guard let sourceName = environment["BRIEFCAL_EVENTKIT_SOURCE"],
              !sourceName.isEmpty,
              let destinationName = environment[
                "BRIEFCAL_EVENTKIT_DESTINATION"
              ],
        ...
        let runID = environment["BRIEFCAL_EVENTKIT_QA_RUN_ID"]
            ?? Self.makeRunID()
        let outputURL = URL(fileURLWithPath:
            environment["BRIEFCAL_EVENTKIT_QA_OUTPUT"]
                ?? NSTemporaryDirectory()
                    .appending("BriefCal-EventKit-QA-\(runID).json")
        )
        let startedAt = Date()
        ...
        print("BRIEFCAL_EVENTKIT_QA_REPORT=\(outputURL.path)")
