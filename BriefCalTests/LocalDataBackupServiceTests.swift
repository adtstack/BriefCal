...
final class LocalDataBackupServiceTests: XCTestCase {
    func testExportCreatesInspectableCurrentSchemaBackup() throws {
        try withTemporaryDirectory { directory in
            ...
            let archiveURL = directory.appendingPathComponent("BriefCal Backup.zip")
            ...
            XCTAssertEqual(result.manifest.applicationIdentifier, "com.adtstack.briefcal")
            ...
            XCTAssertEqual(result.manifest.databaseFilename, "briefcal.sqlite")
    ...
    func testResetDeletesAllBriefCalUserTablesAndAutomaticBackupCanRestoreThem() throws {
        try withTemporaryDirectory { directory in
            let database = try AppDatabase.open(
    ...
    func testBootstrapRecoveryQuarantinesFailedDatabaseFamilyAndInstallsBackup() throws {
        try withTemporaryDirectory { directory in
            ...
            let liveURL = liveDirectory.appendingPathComponent("briefcal.sqlite")
            ...
            XCTAssertEqual(
                try Data(
                    contentsOf: result.quarantinedDatabaseDirectory
                        .appendingPathComponent("briefcal.sqlite")
                ),
                failedDatabase
            )
            for (suffix, data) in sidecars {
                XCTAssertEqual(
                    try Data(
                        contentsOf: result.quarantinedDatabaseDirectory
                            .appendingPathComponent("briefcal.sqlite\(suffix)")
                    ),
                    data
                )
            }
    ...
    func testBootstrapRecoveryRejectsArchiveBeforeTouchingFailedDatabase() throws {
        try withTemporaryDirectory { directory in
            let liveURL = directory.appendingPathComponent("briefcal.sqlite")
            ...
            try runSQLite3(
                ...
                sql: """
                    ...
                    INSERT INTO sqlite_master (
                        ...
                        'trigger',
                        'sqlite_briefcal_hidden',
                        'personal_tasks',
                        0,
                        'CREATE TRIGGER sqlite_briefcal_hidden AFTER INSERT ON personal_tasks BEGIN DELETE FROM grdb_migrations; END'
                    );
            ...
            XCTAssertEqual(
                try database.read { db in
                    try String.fetchOne(
                        ...
                        arguments: ["sqlite_briefcal_hidden"]
                    )
                },
                "sqlite_briefcal_hidden"
            )
            ...
            try db.execute(
                sql: """
                    INSERT INTO provider_pending_operations (
                        ...
                        remote_parent_id, created_at, updated_at
                    ) VALUES (?, ?, ?, 'delete', ?, 'list', ?, ?)
                    """,
                arguments: [
                    "pending-\(suffix)", "account-\(suffix)",
                    "task-\(suffix)", "remote-\(suffix)", timestamp, timestamp
                ]
            )
            try db.execute(
                sql: """
                    ...
                    """,
                arguments: ["task-\(suffix)", timestamp, timestamp]
            )
            try db.execute(
                sql: """
