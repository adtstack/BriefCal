...
final class CalendarEventEditingTests: XCTestCase {
    func testEventKitStoreExecutorRunsReadsOffMainThread() async throws {
        let executor = EventKitStoreExecutor(label: "BriefCalTests.EventKitStore")
    ...
    func testLinkedSingleDeletionPreservesBriefAndCreatesNonUndoableLog() async throws {
        ...
        XCTAssertTrue(retainedTaskItem.wasOriginalDeletedByBriefCal)
    ...
    func testPastDeletionLogAfterRelinkDoesNotLabelLaterCancelledOrphanAsDeletedOriginal() async throws {
        ...
        // adding a BriefCal original-deletion record for the replacement link.
        try database.write { db in
            try db.execute(
                sql: """
        ...
        XCTAssertFalse(item.wasOriginalDeletedByBriefCal)
