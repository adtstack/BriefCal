...
final class ContextStoreTests: XCTestCase {
    ...
    func testProviderTaskSidebarGroupedDetailsFitsAndProducesOffscreenBitmap() async throws {
        ...
        let preferenceSuiteName = "BriefCalTests.TasksSidebar.\(UUID().uuidString)"
        ...
        func render(
            ...
            if width == 360,
               let snapshotPath = ProcessInfo.processInfo.environment[
                "BRIEFCAL_TASKS_SIDEBAR_SNAPSHOT_PATH"
               ] {
        ...
        try await render(
            width: 360,
            height: 700,
            name: "BriefCal Tasks Sidebar 360"
        )
        try await render(
            width: 300,
            height: 600,
            name: "BriefCal Tasks Sidebar 300"
        )
    }

    @MainActor
    func testTaskCenterRowsFitMinimumWorkspaceAndProduceOffscreenBitmap() throws {
        let harness = try makeHarness()
        let planned = try harness.store.personalTasks.create(
            title: "Prepare the quarterly accessibility and migration review",
            dueAt: date(2026, 7, 10, 17)
        )
        let plannedID = TaskCenterItemID.personalTask(taskID: planned.id)
        ...
        _ = try harness.store.taskPlanning.addChecklistItem(
            to: plannedID,
            title: "Review contrast notes"
        )
        _ = try harness.store.personalTasks.create(
            title: "Confirm the release checklist with the whole team",
            dueAt: date(2026, 7, 10, 18)
        )
        ...
        guard case let .loaded(items) = state.taskCenterState else {
            return XCTFail("Expected Task Center fixture to load")
        }
        XCTAssertEqual(items.count, 2)

        let size = CGSize(width: 560, height: 700)
        let hostingView = NSHostingView(rootView:
            TaskCenterView(appState: state)
        ...
        XCTAssertGreaterThanOrEqual(representation.pixelsHigh, 700)
        XCTAssertGreaterThan(pngData.count, 10_000)
        let attachment = XCTAttachment(
            data: pngData,
            uniformTypeIdentifier: "public.png"
        )
        attachment.name = "BriefCal Task Center Minimum Workspace"
        attachment.lifetime = .keepAlways
        add(attachment)
    ...
    @MainActor
    func testEventBriefPrioritizesNotesAtMinimumInspectorWidth() async throws {
        let harness = try makeHarness()
        let event = makeEvent(
            id: "readable-event-brief",
            title: "Quarterly accessibility review and migration readiness",
            start: date(2026, 7, 10, 14),
            end: date(2026, 7, 10, 15)
        )
        _ = try harness.store.saveNotes(
            for: event,
            notes: "Capture the release decision, remaining accessibility risks, and the owner for each migration follow-up."
        )
        _ = try harness.store.appendEventTask(
            for: event,
            section: .before,
            title: "Review the contrast audit"
        )
        _ = try harness.store.appendEventTask(
            for: event,
            section: .during,
            title: "Confirm the migration owner"
        )
        _ = try harness.store.appendEventTask(
            for: event,
            section: .after,
            title: "Publish the decision record"
        )

        let calendarProvider = FakeCalendarProvider(
            authorizationState: .fullAccess
        )
        calendarProvider.calendars = [makeCalendarSource(
            id: event.calendarIdentifier,
            title: "Product Operations"
        )]
        calendarProvider.events = [event]
        let state = AppState(
            calendar: testCalendar,
            now: { harness.now },
            calendarProvider: calendarProvider,
        ...
        guard case .loaded = state.eventBriefState else {
            return XCTFail("Expected Event Brief fixture to load")
        }

        let size = CGSize(width: 300, height: 700)
        let hostingView = NSHostingView(rootView:
            ScrollView {
                EventBriefView(appState: state, event: event)
                    .padding(16)
            }
            .frame(width: size.width, height: size.height)
            .background(Color(nsColor: .windowBackgroundColor))
        ...
        XCTAssertGreaterThanOrEqual(representation.pixelsWide, 300)
        XCTAssertGreaterThanOrEqual(representation.pixelsHigh, 700)
        XCTAssertGreaterThan(pngData.count, 8_000)
        let attachment = XCTAttachment(
            data: pngData,
        ...
        attachment.name = "BriefCal Event Brief Minimum Inspector"
        attachment.lifetime = .keepAlways
        add(attachment)
        window.contentView = nil
    }
    ...
    func testProviderTaskEditorCreateSheetFitsAndProducesOffscreenBitmap() throws {
        ...
        attachment.name = "BriefCal Tasks Create Sheet"
        attachment.lifetime = .keepAlways
        add(attachment)
    }
    ...
    func testProviderTaskEditorDrawerFitsNarrowSidebarAndProducesOffscreenBitmap() throws {
        ...
        if let snapshotPath = ProcessInfo.processInfo.environment[
            "BRIEFCAL_TASK_DRAWER_SNAPSHOT_PATH"
        ] {
            try pngData.write(to: URL(fileURLWithPath: snapshotPath))
        }
        let attachment = XCTAttachment(
        ...
        attachment.name = "BriefCal Tasks Bottom Drawer 340"
        attachment.lifetime = .keepAlways
        add(attachment)
    }

    @MainActor
    func testProviderTaskEditorDisappearFlushesPendingAutoSaveAndDiscardCancelsIt() async throws {
        let debouncer = ProviderTaskEditorAutoSaveDebouncer(
            delayNanoseconds: 100_000_000
        )
        var savedDrafts: [String] = []

        debouncer.schedule {
            savedDrafts.append("pending draft")
        }
        await debouncer.flush {
            savedDrafts.append("unexpected fallback")
        }.value
        XCTAssertEqual(savedDrafts, ["pending draft"])

        await debouncer.flush {
            savedDrafts.append("latest draft")
        }.value
        XCTAssertEqual(savedDrafts, ["pending draft", "latest draft"])

        debouncer.schedule {
            savedDrafts.append("discarded draft")
        }
        debouncer.cancel()
    ...
    @MainActor
    func testProviderTaskEditDrawerPrioritizesTitleAndNotesAndProducesOffscreenBitmap() async throws {
        let harness = try makeHarness()
        let list = RemoteTaskList(
            provider: .appleReminders,
            id: "icloud-work",
        ...
        let snapshot = RemoteTaskSnapshot(
            ...
            title: "Review the launch checklist and confirm every owner before publishing",
            notes: "Check migration, accessibility, release notes, and the final customer communication.",
            dueAt: date(2026, 7, 18, 9),
            isCompleted: false,
            version: "1",
            deepLink: nil
        ...
        for width: CGFloat in [300, 340, 360] {
            let hostingView = NSHostingView(rootView:
                ProviderTaskEditorSheet(
                    coordinator: coordinator,
                    mode: .edit(item),
            ...
            XCTAssertGreaterThan(pngData.count, 6_000)
            let attachment = XCTAttachment(
                data: pngData,
                uniformTypeIdentifier: "public.png"
            )
            attachment.name = "BriefCal Task Details Drawer \(Int(width))"
            attachment.lifetime = .keepAlways
            add(attachment)
            if width == 340,
               let snapshotPath = ProcessInfo.processInfo.environment[
                "BRIEFCAL_TASK_DETAIL_SNAPSHOT_PATH"
               ] {
                try pngData.write(to: URL(fileURLWithPath: snapshotPath))
            }
            window.contentView = nil
    ...
    func testTaskCenterProjectsProviderConflictAndExplicitRecoveryChoices() async throws {
        ...
        let remote = RemoteTaskSnapshot(
            ...
            notes: "Remote body stays outside BriefCal",
            dueAt: nil,
            isCompleted: true,
            version: "remote-v2",
            deepLink: nil
        ...
        XCTAssertThrowsError(
            try OAuthAuthorizationRequest.make(
                configuration: OAuthProviderConfiguration(
                    provider: .googleTasks,
                    clientID: "client",
                    redirectURI: try XCTUnwrap(URL(string: "briefcal://oauth"))
                ),
                pkce: request.pkce
            )
        )
    ...
    func testLegacySyntheticSingleSnapshotDriftRequiresConfirmation() throws {
        ...
        let changedSingle = makeEvent(
            id: "legacy-mismatch",
            title: "Changed outside BriefCal",
    ...
    func testFileBackedDatabaseReopensWithAllPhaseThreeData() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("BriefCal-ContextStore-\(UUID().uuidString)")
